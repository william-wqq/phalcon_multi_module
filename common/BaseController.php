...
class BaseController extends Controller
{
    ...
    {
        if (isset($name) && $name !== '') {
            $value = $this->post($name, $filter, null, $noRecursive) ?? $this->get($name, $filter, null, $noRecursive);
            return $value ?? $this->post($name, $filter, $defaultValue, $noRecursive);
        }
     ...
     *            'App\Home\Controllers\forward/index/a/aaa?b=bbb' or 'forward/index/a/aaa?b=bbb' or 'index?b=bbb'
     * @param array|string $vars
     *            参数 ['a'=>'aaa','b'=>'bbb'] or 'a=aaa&b=bbb'
     ...
     * 响应成功json数据
     *
     * @param string $rspMessage 成功提示
     * @param array $rspData  返回数据
     * @param int $status 返回状态
     * @param int $jsonOptions json选项
     * @param int $depth 处理数组深度
     * @return object
     */
    protected function sendSuccess(string $rspMessage = null, array $rspData = null, int $status = 1, int $jsonOptions = null, int $depth = 512)
    {
        $responseData = [
            'status' => $status,
            'message' => $rspMessage ?? 'success',
            'data' => $rspData ?? []
        ];
     ...
     * @return object
     */
    protected function sendError(string $rspMessage, array $rspData = null, int $status = 0, int $jsonOptions = null, int $depth = 512)
    {
        $responseData = [
            'status' => $status,
            'message' => $rspMessage ?? 'error',
            'data' => $rspData ?? []
        ];
        return $this->response->setJsonContent($responseData, $jsonOptions, $depth)->send();
    }
